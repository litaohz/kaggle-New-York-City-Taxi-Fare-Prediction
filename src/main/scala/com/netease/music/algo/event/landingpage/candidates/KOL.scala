...
object KOL {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val popStar = spark.read.option("sep", "\001").
      schema(commonSchema).csv("/user/ndir/hive_db/ndir_201712_unicom_tmp.db/music_kol/ds=".concat(args(0))).
      select("result").withColumnRenamed("result", "creatorId")
    /*
    pair:
     */
     videoPref.
    ...
    result.withColumn("friendid", appendStr(str)($"friendid")).
      ...
      write.mode(SaveMode.Overwrite).option("compression", "gzip").text(landingPageOutputKOL.concat(args(0)))
