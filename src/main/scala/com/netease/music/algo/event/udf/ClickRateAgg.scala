...
class ClickRateAgg extends UserDefinedAggregateFunction {
  def inputSchema: org.apache.spark.sql.types.StructType =
    StructType(StructField("friendid", StringType) :: StructField("impress", LongType) :: StructField("click", LongType) :: Nil)
  ...
  def update(buffer: MutableAggregationBuffer, input: Row): Unit = {
    val map = new scala.collection.mutable.HashMap[String, Tuple2[Int, Int]]
    val str = buffer.getAs[String](0)
    val friendid1 = input.getAs[String](0)
    val imp1 = input.getAs[Int](1)
    val clk1 = input.getAs[Int](2)
    map.put(friendid1, (imp1, clk1))
    str.split(",").map(getX12).foreach(x => updateMap4(x._1, x._2, map))
